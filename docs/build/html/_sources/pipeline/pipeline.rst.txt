...
requires being able to use both the old ``ms3 1.2.11`` and the latest version in alternation, e.g. using virtual
environments. To date, it also requires access to DCML's private repos.
...
* If there are branches with work in progress (in the screenshot, ``scene_0_workflow_update`` and
  ``scene_1_workflow_update``), we need to be extra careful to take the right decision and to check with the author(s).
  Several scenarious are possible:
